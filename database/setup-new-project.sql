/*
  KH Invoice — Complete Database Setup for a NEW Supabase Project
  ==================================================================
  Run this ONE file in your new Supabase project's SQL Editor
  (Dashboard → SQL Editor → New query → paste → Run) and you're done.
  It is the consolidated schema (through 2026-07-20) plus the later
  PayWay QR-integration columns (2026-07-22), merged in the correct
  order. Safe to re-run (every statement uses IF NOT EXISTS / OR REPLACE
  / DROP POLICY IF EXISTS).

  After running this:
  1. Project Settings → API → copy the Project URL and anon public key
     into your .env file (see .env.example).
  2. Deploy the two Edge Functions in supabase/functions/ (or paste the
     self-contained copies from supabase/functions-dashboard-copy/
     directly into the Dashboard's Edge Functions editor).
  3. (Optional, only for automatic PayWay QR) Edge Functions → Secrets:
     set PAYWAY_MERCHANT_ID and PAYWAY_API_KEY.
*/
...
-- ============================================================
-- Added 2026-07-22: PayWay automatic-QR integration columns
-- ============================================================
/*
